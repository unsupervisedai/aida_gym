 ...
 #   self._env_randomizer = env_randomizer
  ...
  def _step(self, action):
    ...
    if self._is_render:
      ...
      #print(len(base_pos))
      #print(self.aida.GetObservation())
    ...
    for _ in range(self._action_repeat):
      ...
      self._pybullet_client.stepSimulation()
    #print(self.aida.GetMotorAngles()[2])
    #base_pos = self.aida.GetBasePosition()
    #print(base_pos[2])
    #print(111111111111111111)
    ...
    return np.array(self._noisy_observation()), reward[0], done, {},reward[1],reward[2],reward[3],reward[4],reward[5],reward[6]
  ...
  def _reward(self):
    ...
    if current_base_position[2] > 0.6:
      height_reward = 1
    else:
      height_reward = 0
#    if self.aida.GetMotorAngles()[1]>-0.15 or self.aida.GetMotorAngles()[4]>-0.15 or #self.aida.GetMotorAngles()[7]>-0.15 or self.aida.GetMotorAngles()[10]>-0.15:
    if self.aida.GetMotorAngles()[2]>0.25 or self.aida.GetMotorAngles()[11]>0.25 or self.aida.GetMotorAngles()[5]<0.25 or self.aida.GetMotorAngles()[8]<0.25:
      back_rew=1;
    else:
      back_rew=0;
    ...
    reward = (
        ...
        + self._shake_weight * shake_reward + self._height_weight * height_reward-self._back_weight*back_rew)
    self._objectives.append(
        [forward_reward, energy_reward, drift_reward, shake_reward, height_reward])
    return [reward,self._distance_weight * forward_reward,self._energy_weight * energy_reward,self._drift_weight * drift_reward,self._shake_weight * shake_reward,self._height_weight * height_reward,self._back_weight*back_rew]
